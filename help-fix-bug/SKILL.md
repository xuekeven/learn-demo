---
name: help-fix-bug
description: 系统性排查并修复缺陷：澄清现象与复现、收集证据（日志/报错/环境）、缩小范围、提出最小修复并验证。在用户以 /help-fix 开头（可接问题描述、禅道/Jira/前端监控警告的链接、或 https://*.jd100.com/* 产品链接），或说「帮我看看这个 bug」「帮我看看这个问题」「帮忙排查一下」「看下这个报错」等求助排查/修复表达（可接同上描述或链接）时触发。
...
- 前端监控警告：`https://fe-monitor.jd100.com/easytech/issues/*?project=*`（`issues/` 后 issue id 与 `project` 参数可变）  

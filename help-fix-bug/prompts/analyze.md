...
```
现在开始分析 bug。首先从 ${bug_appear_baseline} 新建 fix-${实时年月日}-${实时时分} 分支，在该分支上分析和修复代码。
```
...
分析完成之后，不用复述 bug 详情，只向用户输出原因、证据链和复现路径，要求用户手动复现 bug 并询问 bug 原因是否正确，如果正确，进入下一流程；否则根据用户的回答，视情况进一步分析（分析方向正常）或者重新从头分析（分析方向错误），继续查找和确定原因。
